...
class Data():
    ...
    def __init__(self, xpr=lambda x: 0, num=0):
        self.xpr = xpr
        self.num = num
    ...
    def __xor__(self, rhs):
        if type(rhs) == int:
            ...
            if self.isNumType():
                return Data(xpr=lambda x: (self.num) ^ (rhs.xpr(x) + rhs.num))
            else:
                return Data(xpr=createLm(7, self.xpr, self.num, rhs.xpr, rhs.num))
    ...
    def rotateRight(self, x):
        x = x % 32
        if self.isNumType():
            return Data(num=rotate_right(self.num))
        else:
    ...
    def rotateLeft(self, x):
        x = x % 32
        if self.isNumType():
            return Data(num=rotate_left(self.num))
        else:
