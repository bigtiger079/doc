...
class JEB2AutoRenameByTypeInfo(IScript):
    ...
    def run(self, ctx):
        ...
        ctx.executeAsync("Running name detection...", JEBCheckArgs(ctx))
        ...
        # current_addr = fragment.getActiveAddress()
        # print(" address: " + current_addr)
        # current_item = fragment.getActiveItem()
        # print("    item: %s" % current_item)
        # print("itemText: " + fragment.getActiveItemAsText())
        print('Done')


class JEBCheckArgs(Runnable):
    ...
    def run(self):
        ...
        for unit in units:
            ...
            print(dir(unit))
            # document = unit.getSourceDocument()
            # print('anchoreCount: %d' % document.getAnchorCount())
            ...
            for mt in methods:
                ...
                if paramsCount == 0:
                    continue
                if mt.getName() == '<init>' and paramsCount == 1:
                    continue
                for param in params:
                    name = param.getIdentifier().getName()
                    if name == 'this':
                        continue
                    print('%s -> %s, %s' % (param.getType().getSignature(), name,  param.getElementType()))
                   
                    if name == 'Landroid/content/Context;':
                        data = ActionRenameData()
                        data.setNewName('context')
                        # context = ActionContext(unit, Actions.RENAME, current_item.getItemId(), current_addr)
...
def description(unit):
    if unit.getParent() != None:
        return unit.toString() + " -> " + description(unit.getParent())
    else:
