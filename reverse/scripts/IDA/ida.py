...
def find_data(addr):
    ...
    if opndcode.startswith('ADD'):
        if idc.GetOpType(def_addr, 2) == 0:
            return parse_register_value('R0', def_addr,
                                        func_head) + parse_opnd_value(
                                            def_addr, func_head, 1)
        else:
            return parse_opnd_value(def_addr, func_head, 1) + parse_opnd_value(
                def_addr, func_head, 2)
...
def find_opnd_def_addr(opnd, from_addr, to_addr):
    ...
    while _addr > to_addr:
        ...
        if idc.GetOpType(_addr, 0) == 1 and _r0 == opnd:
            if _opcode.startswith('ADD') or _opcode.startswith(
                    'MOV') or _opcode.startswith('LDR'):
                return _addr
...
def parse_opnd_value(addr, end_addr, position):
    ...
    if type == 0:
        ...
        return parse_register_value(idc.GetOpnd(addr, position), addr,
                                    end_addr)
    elif type == 2:
        ...
        if m is not None:
            ...
            print "passe error -> %s at 0x%x" % (idc.GetOpnd(addr,
                                                             position), addr)
    elif type == 4:
        return find_last_str_operation(idc.GetOpnd(addr, position), addr,
                                       end_addr)
    elif type == 5:
